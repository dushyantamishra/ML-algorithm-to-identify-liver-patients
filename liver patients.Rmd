---
title: "Identification of Liver Patients"
...
Patients in hospitals often exhibit complex medical expositions, making it difficult for doctors to determine whether any undiagnosed underlying conditions may be impacting their current condition.

For example, when a patient shows symptoms like aches, 
dizziness or tiredness, they may be examined and diagnosed for an underlying heart conditon. While this is likely to be the most probable and significant conditon they have, it is possible that they might have another condition which is aggravating their symptoms.

In recent years, there has been an increase in patients being diagnosed with chronic liver disease, which requires the expertise of a hepatologist. 

For this reason, this project attempts to develop a machine learning technique that identifies those patients who are similar to patients who receive care from a hepatologist. The dataset used is available at UCI Machine Learning Repository,
http://archive.ics.uci.edu/ml.



The aimed result of the project is an algorithm that could be run 
in the background against blood test results to indicate whether patients who are not currently under hapatic care would benefit from further examination and potentially a referral for continuing care.
...
The data is downloaded through the following code, the comments after the column names are from the original dataset's information page. 
...
Next, the data is split into training and test sets, noting the approximate proportions in the outcome column.
...
With a broad overview, both groups of patients
...
It appears that women appear less frequently than men in both groups (group with liver disease and the control group). The control group looks as though it may have been constructed to mimic the patients with liver disease.
